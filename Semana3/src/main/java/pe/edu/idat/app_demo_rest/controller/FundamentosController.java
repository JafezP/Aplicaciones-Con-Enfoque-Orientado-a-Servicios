...
public class FundamentosController {
    ...
    @GetMapping("/primo")
    public ResponseEntity<PrimoDto> validarNumeroPrimo(@RequestParam Integer numero) {
        PrimoDto primoDto = new PrimoDto();

        primoDto.setNumero(numero);
        primoDto.setResultado(fundamentosService.validarNumeroPrimo(numero));

        return ResponseEntity.ok(primoDto);
    }

    //localhost:8055/api/v1/operaciones/factorial?=numero=5
    @GetMapping("/factorial")
    public ResponseEntity<FactorialDto> validarFactorial(@RequestParam(required = false, defaultValue = "2") Integer numero) {
        FactorialDto factorialDto = new FactorialDto();

        factorialDto.setNumero(numero);
        factorialDto.setValorFactorial(fundamentosService.calcularFactorial(numero));

        return ResponseEntity.ok(factorialDto);
    }

    @GetMapping({"/fibonacci", "/fibonacci/{numero}"})
    public ResponseEntity<FibonacciDto> calcularFibonacci(@PathVariable(required = false) Integer numero) {
        if (numero == null) numero = 1;
        FibonacciDto fibonacciDto = new FibonacciDto();

        fibonacciDto.setCantidadNumero(numero);
        fibonacciDto.setFibonacci(fundamentosService.calcularFibonacci(numero));
